	...
	for (const line of lines) {
		...
		switch (type) {
			...
			case 'tool.execution_complete': {
				...
				const toolName = bufferedStart?.toolName || bufferedRequest?.name || 'unknown';
				const toolArgs = bufferedStart?.arguments || bufferedRequest?.arguments || {};
				...
				if (bufferedStart) {
					const durationMs = timestamp.getTime() - bufferedStart.timestamp.getTime();
					if (durationMs > 0) {
						resultMeta.duration = durationMs >= 1000
				...
				if (metrics) {
					if (metrics.linesAdded !== undefined) resultMeta.linesAdded = metrics.linesAdded;
					if (metrics.linesRemoved !== undefined) resultMeta.linesRemoved = metrics.linesRemoved;
				}

				// Extract file paths from telemetry or arguments
				const restricted = telemetry?.restrictedProperties as Record<string, string> | undefined;
