...
<div
  class="rounded-lg bg-surface-card/60 overflow-hidden border {isActive ? 'border-accent/60 bg-accent/5' : 'border-accent/20'} transition-colors"
>
  <!-- Header bar -->
  <button
  ...
  >
    ...
    <span class="text-foreground-bright font-medium"
      >{entry.description || entry.subagentType}</span
    >
  ...
  {#if expanded}
    <div class="border-t border-edge/30 flex text-xs">
      ...
      <div
        class="w-48 shrink-0 px-3 py-2.5 border-r border-edge/30 space-y-1.5"
      >
        ...
        {#if entry.tokens?.total}
          <div class="flex justify-between">
            ...
            <span class="text-foreground"
              >{formatTokens(entry.tokens.total)}</span
            >
          </div>
        {/if}
        <div class="flex justify-between">
      ...
      <div class="flex-1 px-3 py-2.5 min-w-0 overflow-hidden">
        ...
        {#if resultPreview}
          <pre
            class="text-foreground/80 whitespace-pre-wrap break-words text-xs leading-relaxed"
          >{resultPreview}</pre>
