...
<div
  class="rounded-lg bg-surface-card/60 overflow-hidden opacity-70 hover:opacity-100 transition-opacity"
>
  ...
  >
    ...
    <span class="text-foreground-bright font-medium"
      >{entry.displayName || entry.name}</span
    >
      ...
      >
        ...
        {#if meta?.exitCode !== undefined}
          <div class="flex justify-between">
            ...
            <span
              class="{meta.exitCode === 0 ? 'text-status-success' : 'text-status-error'}"
              >{meta.exitCode}</span
            >
          </div>
        {/if}
