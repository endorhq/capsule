...
class GitMasker {
  ...
  maskInString(text: string): string {
    ...
    let result = text;
    const entries = [
      ...this.branchMap.entries(),
      ...this.repoMap.entries(),
...
function anonymizeClaudeLine(
  ...
  if (
    options.removeSystemMessages &&
    (type === 'system' || type === 'file-history-snapshot')
  ) {
    return null;
  }
    ...
    if (Array.isArray(content)) {
      msg.content = content
        ...
        .map(block => {
          ...
          if (
            options.maskFilePaths &&
            block.type === 'tool_use' &&
            block.input
          ) {
            block.input = maskPathsInObject(
  ...
  if (type === 'user' && entry.message && typeof entry.message === 'object') {
    ...
    if (options.removeToolOutputs && entry.toolUseResult) {
      ...
      entry.toolUseResult = maskPathsInObject(
...
function anonymizeCodexLine(
  ...
  if (type === 'session_meta') {
    ...
    if (options.maskGitInfo && payload.git) {
      ...
      if (git.repository_url)
        git.repository_url = gitMasker.maskRepo(git.repository_url as string);
    }
  }
  ...
  if (type === 'response_item') {
    ...
    if (
      ...
      if (payload.arguments && typeof payload.arguments === 'string') {
        try {
          ...
          payload.arguments = JSON.stringify(
            maskPathsInObject(args, pathMasker)
          );
        } catch {
          // ignore
        }
    ...
    if (options.maskFilePaths && itemType === 'message') {
      const contentArr = payload.content as
...
function anonymizeCopilotLine(
  ...
  if (type === 'session.start') {
    ...
    if (ctx) {
      ...
      if (options.maskGitInfo) {
        ...
        if (ctx.repository)
          ctx.repository = gitMasker.maskRepo(ctx.repository as string);
      }
    }
  }
  ...
  if (type === 'tool.execution_start') {
    if (
      ...
      data.arguments = maskPathsInObject(
        ...
        if (toolCalls) {
          msg.toolCalls = toolCalls.map(tc => {
            if (options.removeToolOutputs) {
              ...
              const resultArr = tc.result as
                | Array<Record<string, unknown>>
                | undefined;
              if (resultArr) {
                for (const r of resultArr) {
                  const funcResp = r.functionResponse as
