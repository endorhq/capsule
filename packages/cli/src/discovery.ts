...
export async function discoverAllSessions(): Promise<AgentSource[]> {
	...
	const checks: Array<{ agent: AgentFormat; label: string; basePath: string; discover: () => Promise<DiscoveredSession[]> }> = [
		{
			agent: 'claude',
			label: 'Claude Code',
		...
		},
		{
			...
			discover: () => discoverCopilotSessions(join(home, '.copilot', 'session-state'))
		},
		{
			agent: 'gemini',
			label: 'Gemini CLI',
 ...
 */
async function peekLines(filePath: string, maxLines: number): Promise<string[]> {
 ...
 */
async function peekClaudeSession(filePath: string): Promise<DiscoveredSession | null> {
	try {
		...
		for (const line of lines) {
			try {
				...
				if (!title && entry.type === 'user' && entry.message?.content && typeof entry.message.content === 'string') {
 ...
 */
async function peekCodexSession(filePath: string): Promise<DiscoveredSession | null> {
	try {
		...
		for (const line of lines) {
			try {
				...
				if (entry.type === 'event_msg' && entry.payload?.type === 'user_message') {
					hasUserMessage = true;
					if (!title) {
						const firstLine = (entry.payload.message as string || '').trim().split('\n')[0].trim();
						title = truncate(firstLine, 80);
					}
				}
			} catch {
 ...
 */
async function peekCopilotSession(filePath: string): Promise<DiscoveredSession | null> {
	try {
		const lines = await peekLines(filePath, 20);
		if (lines.length === 0) return null;
		...
		for (const line of lines) {
			try {
				...
				if (!title && entry.type === 'user.message' && entry.data?.content) {
					const firstLine = (entry.data.content as string).trim().split('\n')[0].trim();
					title = truncate(firstLine, 80);
				}
			} catch {
				continue;
		...
		return {
			agent: 'copilot',
			filePath,
			sessionId: sessionId || basename(filePath.replace(/\/events\.jsonl$/, '')),
 ...
 */
async function peekGeminiSession(filePath: string): Promise<DiscoveredSession | null> {
	try {
		const content = await readFile(filePath, 'utf-8');
		...
		const messages = root.messages as Array<{ type: string; content?: string }> | undefined;
		let title = '';
		if (messages) {
...
async function discoverClaudeSessions(basePath: string): Promise<DiscoveredSession[]> {
...
async function discoverCodexSessions(basePath: string): Promise<DiscoveredSession[]> {
	const sessions: DiscoveredSession[] = [];
	try {
...
async function discoverCopilotSessions(basePath: string): Promise<DiscoveredSession[]> {
	const sessions: DiscoveredSession[] = [];
	try {
...
async function discoverGeminiSessions(basePath: string): Promise<DiscoveredSession[]> {
	const sessions: DiscoveredSession[] = [];
	try {
		const entries = await readdir(basePath, { recursive: true, withFileTypes: true });
		for (const entry of entries) {
