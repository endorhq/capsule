...
async function main() {
	...
	if (!fileContent) {
		...
		if (sources.length === 1) {
			...
			p.log.info(`Found ${pc.cyan(String(selectedSource.sessionCount))} ${selectedSource.label} sessions`);
		} else {
	...
	const anonChoices = await p.multiselect({
		...
		options: (Object.keys(ANONYMIZE_OPTION_LABELS) as Array<keyof AnonymizeOptions>).map((key) => ({
			value: key,
	...
	const outputChoice = await p.select({
		...
		options: [
			{ value: 'gist', label: 'Publish to GitHub Gist', hint: 'creates a shareable link' },
