...
import type { AgentSource, DiscoveredSession } from '../types/discovery.js';
import type { AgentFormat } from '../types/timeline.js';

export type { AgentSource, DiscoveredSession } from '../types/discovery.js';
...
/**
 * Validate that a file path is under one of the known agent directories.
 * Prevents path traversal attacks when serving local files.
 */
export function isKnownAgentPath(filePath: string): boolean {
  const home = homedir();
