  ...
  for (const msg of messages) {
    ...
    if (tokens) {
      ...
      totalTokens.reasoning =
        (totalTokens.reasoning || 0) + (tokens.thoughts || 0);
    ...
    if (toolCalls && toolCalls.length > 0) {
      for (const tc of toolCalls) {
        ...
        const toolDisplayName =
          (tc.displayName as string) || extractToolDisplayName(toolName);
        const args = (tc.args as Record<string, unknown>) || {};
        const status =
          (tc.status as string) === 'success' ? 'success' : 'error';
        const tcTimestamp = tc.timestamp
          ? new Date(tc.timestamp as string)
          : timestamp;
        ...
        if (resultArr && resultArr.length > 0) {
          const funcResp = resultArr[0].functionResponse as
        ...
        const resultDisplay = tc.resultDisplay as
          | Record<string, unknown>
          | undefined;
        if (
          resultDisplay &&
        ...
        // File path from args
        const filePath =
          (args.file_path as string) || (args.path as string) || '';
        if (filePath) {
        ...
        if (filePath) {
          ...
          if (toolName === 'read_file') {
            ...
            if (
              resultMeta.linesAdded !== undefined ||
              resultMeta.linesRemoved !== undefined
            ) {
              parts.push(
