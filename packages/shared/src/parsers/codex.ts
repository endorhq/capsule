  ...
  if (name === 'exec_command') {
    ...
    try {
      ...
      if (parsed.metadata?.exit_code !== undefined)
        meta.exitCode = parsed.metadata.exit_code;
      ...
      if (fileMatches) {
        meta.files = fileMatches.map((m: string) =>
          m.replace(/^[MAD]\s+/, '').trim()
        );
      }
    } catch {
      meta.output = output;
      ...
      case 'response_item': {
        ...
        if (itemType === 'message') {
          ...
          const text =
            contentArr
              ?.map(
                c =>
                  (c.text as string) ||
                  (c.type === 'output_text' ? (c.text as string) : '')
              )
              .filter(Boolean)
              .join('\n') || '';
          ...
          if (buffered) {
            ...
            const resultMeta = parseResultMeta(
              buffered.name,
              buffered.arguments,
              output
            );
