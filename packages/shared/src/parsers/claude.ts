...
export function parseClaudeSession(content: string): ParsedSession {
  ...
  for (const line of lines) {
    ...
    if (entryType === 'progress') {
      ...
      const progressMessage = data.message as
      ...
      if (!pendingSubagents.has(agentId)) {
        ...
        const taskTool = parentToolUseID
          ? pendingToolCalls.get(parentToolUseID)
          : undefined;
      ...
      if (msgType === 'assistant' && innerMessage) {
        ...
        const msgTimestamp = new Date(
          (progressMessage.timestamp as string) || (entry.timestamp as string)
        );
        parseSubagentAssistantBlocks(
          sub,
          contentBlocks,
          msgTimestamp,
          usage,
        ...
        if (Array.isArray(msgContent)) {
          parseSubagentUserBlocks(
            sub,
            msgContent as ContentBlock[],
            msgTimestamp
          );
      ...
      if (Array.isArray(msgContent)) {
        ...
        for (const block of msgContent as ContentBlock[]) {
          ...
          if (filePath) {
            ...
            if (toolName === 'Read') {
              const file = toolUseResult?.file as
                | Record<string, unknown>
                | undefined;
              const numLines = file?.numLines as number | undefined;
              summary = numLines
            ...
            summary =
              numFiles !== undefined ? `${numFiles} files found` : undefined;
          }
    ...
    if (entryType === 'assistant') {
      ...
      if (usage) {
        ...
        totalTokens.output =
          (totalTokens.output || 0) + (usage.output_tokens || 0);
